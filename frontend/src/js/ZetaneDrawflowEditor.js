class Drawflow {
  ...
  addNode_from_JSON(specs_json, pos_x, pos_y) {
    var newNodeId = null;
    if (this.useuuid) {
    ...
    const keys = Object.keys(specs_json);
    const root = specs_json;

    let html_str = ""
    let params = root.parameters;
    let first_key;

    if (root.views.node.html != "") {
      ...
      let doc = parser.parseFromString(root.views.node.html, 'text/html')
    ...
    if (root.views.node.title_bar && root.views.node.title_bar.background_color) {
      title_background_color = root.views.node.title_bar.background_color;
    }

    var preview = "";
    if (root.views.node.preview && root.views.node.preview.active && root.views.node.preview.active == 'true') {
      preview =`
      <style>
        .iframe_preview {
            transform: scale(0.5);
    ...
    <div>    
      <div class="title-box" style="display: flex; align-items: center;background-color:`+ title_background_color +`;"> 
          ${root.information.name}
          <div style="margin-left: auto; display: flex;">
              <button class="view-btn" onclick="openCode('../../`+ root.action.block_source + `/computations.py')">
                  <div>
    ...
    const name = root.information.id;
    // const num_in =10;
    // const num_out=6;
    const ele_pos_x = pos_x;
    const ele_pos_y = pos_y;
    const classoverride = root.information.id
    const data = {}
    const html = html_title;


    const input_variable_names = Object.keys(root.inputs);
    const input_types = input_variable_names.map(v => root.inputs[v].type)

    const output_variable_names = Object.keys(root.outputs);
    const output_types = output_variable_names.map(v => root.outputs[v].type)
    ...
    const json_inputs = {}
    let input_max = 0;
    for (var x = 0; x < input_variable_names.length; x++) {
    ...
    const json_outputs = {}
    let output_max = 0;
    for (var x = 0; x < output_variable_names.length; x++) {
      ...
      label.style.left = (-width_of_output - 50) + 'px';
  ...
  convert_drawflow_to_block() {
    ...
    for (let i = 0; i < node_keys.length; i++) {
      ...
      for (let j = 0; j < block_variable_keys_inputs.length; j++) {
        const block_output_connections = this.renameKeyInArrayOfObjects(nodes_inputs[j].connections, 'input', 'output');        
      ...
      for (let j = 0; j < block_variable_keys_outputs.length; j++) {
        const block_input_connections = this.renameKeyInArrayOfObjects(nodes_outputs[j].connections, 'output', 'input');        
        blocks[i][node_keys[i]].outputs[block_variable_keys_outputs[j]].connections = block_input_connections;
      }

      const block_variable_keys_parameters = Object.keys(blocks[i][node_keys[i]].parameters)
      for (let j = 0; j < nodes_parameters.length; j++) {

        blocks[i][node_keys[i]].parameters[block_variable_keys_parameters[j]]['value'] = nodes_parameters[j];
      }
    }

    let graph = {}
    for (let i = 0; i < node_keys.length; i++) {
      graph[node_keys[i]] = blocks[i][node_keys[i]]
    }

    //TO ADD NODE KEY TO ID
    for (let i = 0; i < node_keys.length; i++) {
      graph[node_keys[i]].information.id += '-' + node_keys[i]
    }
    return graph;
  }

  import_block(data, notifi = true) {
    // this.clear();
    this.drawflow_block = JSON.parse(JSON.stringify(data));
    this.load_block();
    if (notifi) {
