...
function removeDanglingConnections(pipeline, blockFolderName, content, editor) {
  getRemovedIOKeys(pipeline[blockFolderName].outputs, content.outputs).forEach(
    (key) => {
      editor.removeNodeOutputConnections(blockFolderName, key);
    },
  );

  getRemovedIOKeys(pipeline[blockFolderName].inputs, content.inputs).forEach(
    (key) => {
      editor.removeNodeInputConnections(blockFolderName, key);
    },
  );
}
