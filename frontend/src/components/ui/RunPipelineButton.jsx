  ...
  const mutation = useMutation({
    mutationFn: async (execution) => {
      const response = await fetch(
        `http://${configuration.host}:${configuration.anvilPort}/execute`,
        {
          method: "POST",
          headers: {
...
export default function RunPipelineButton({ modalPopper, children, action }) {
  ...
  const postExecution = usePostExecution(
    queryClient,
    configuration,
    setWorkspace,
    loadServerPipeline,
  ...
  const runPipeline = async (editor, pipeline) => {
    ...
    let pipelineSpecs = editor.convert_drawflow_to_block(
      pipeline.name,
      pipeline.data,
    );
    ...
    try {
      ...
      setValidationErrorMsg([
        `Failed to upload files to anvil server: ${error}`,
      ]);
      setIsOpen(true);
      return null;
    }
