export default class Drawflow {
  ...
  click(e) {
    if (this.editor_mode === "fixed") {
      ...
      if (e.target.classList[0] == "input-element") {
        ...
        e.target.classList[0] != "output" &&
        e.target.closest(".drawflow-node") != null
      ) {
    ...
    if (["input", "output"].includes(this.ele_selected.classList[0])) {
      // remove 'main-path' -> prevented keydown and deleting connection svg
      e.preventDefault();
    }
  }
    ...
    for (let svg in this.connection_list) {
      ...
      if (!domElement) {
        ...
        this.updateConnectionNodes(
          "node-" + this.connection_list[svg].output_id,
        );
        this.updateConnectionNodes(
          "node-" + this.connection_list[svg].input_id,
        );
      }
    }
  }
    ...
    if (Object.keys(nodeRefs).length) {
      ...
      Object.keys(elemsOut).map(function (item) {
        ...
        const checkAndAssignElements = (
          ...
          if (
            (!inputRef && input_id && input_class) ||
            (!outputRef && output_id && output_class)
          ) {
            console.warn(
              `Node reference not found for input ${input_id}-${input_class} or output ${output_id}-${output_class}`,
            );
            return false;
          }
        ...
        if (elemsOut[item].querySelector(".point") === null) {
          ...
          const { input_class, input_id, output_class, output_id } =
            connection_list[svgName];
          if (
            !checkAndAssignElements(
              input_id,
              input_class,
              output_id,
              output_class,
          ...
          points.forEach((item, i) => {
            ...
            var eX;
            const svgName =
              "." + item.parentElement.classList.value.split(" ").join(".");
            const { input_class, input_id, output_class, output_id } =
              connection_list[svgName];
            ...
            if (i === 0 && points.length - 1 === 0) {
              ...
              eX =
                (circleElement.getBoundingClientRect().x -
                  precanvas.getBoundingClientRect().x) *
                  precanvasWitdhZoom +
                rerouteWidth;
              eY =
                (circleElement.getBoundingClientRect().y -
                  precanvas.getBoundingClientRect().y) *
                  precanvasHeightZoom +
                rerouteWidth;
              ...
              if (
                !checkAndAssignElements(null, null, output_id, output_class)
              ) {
                return;
              }
      ...
      Object.keys(elems).map(function (item) {
        ...
        if (elems[item].querySelector(".point") === null) {
          const svgName =
            "." + elems[item].classList.value.split(" ").join(".");
          const { input_class, input_id, output_class, output_id } =
            connection_list[svgName];
          if (
            !checkAndAssignElements(
  ...
  createReroutePoint(ele) {
    ...
    this.updateConnectionList((draft) => {
      const svgName =
        "." + ele.parentElement.classList.value.split(" ").join(".");
  ...
  removeNodeInputConnections(id, input_class) {
    ...
    Object.keys(infoNode.inputs[input_class].connections).map(
      function (key, index) {
        ...
        const output_class =
          infoNode.inputs[input_class].connections[index].variable;
  ...
  removeNodeOutputConnections(id, output_class) {
    ...
    Object.keys(infoNode.outputs[output_class].connections).map(
      function (key, index) {
        const id_input =
          infoNode.outputs[output_class].connections[index].block;
        const input_class =
          infoNode.outputs[output_class].connections[index].variable;
  ...
  removeConnection() {
    if (this.connection_selected != null) {
      const svgName =
        "." +
        Array.from(this.connection_selected.parentElement.classList).join(".");
      const { input_id, input_class, output_id, output_class } =
        this.connection_list[svgName];
      ...
      this.setPipeline((draft) => {
        const index_out = draft.data[output_id].outputs[
          output_class
        ].connections.findIndex((element) => {
          return element.variable === input_class && element.block === input_id;
        });
        const index_in = draft.data[input_id].inputs[
          input_class
        ].connections.findIndex((element) => {
          return (
            element.variable === output_class && element.block === output_id
          );
        });
        draft.data[output_id].outputs[output_class].connections.splice(
          index_out,
          1,
        );
        draft.data[input_id].inputs[input_class].connections.splice(
          index_in,
          1,
        );
      });
  ...
  removeSingleConnection(id_output, id_input, output_class, input_class) {
    const exists = this.pipeline.data[id_output].outputs[
      output_class
    ...
    if (exists > -1) {
      ...
      this.setPipeline((draft) => {
        var index_out = draft.data[id_output].outputs[
          output_class
        ].connections.findIndex(function (item) {
          return item.block == id_input && item.variable === input_class;
        });
        draft.data[id_output].outputs[output_class].connections.splice(
          index_out,
          1,
        );

        var index_in = draft.data[id_input].inputs[
          input_class
        ].connections.findIndex(function (item) {
          return item.block == id_output && item.variable === output_class;
        });
        draft.data[id_input].inputs[input_class].connections.splice(
          index_in,
          1,
        );
      });
    ...
    this.setPipeline((draft) => {
      ...
      for (let connection in this.connection_list) {
        if (connection.includes(nodeId)) {
          const { input_id, input_class, output_id, output_class } =
            this.connection_list[connection];
  ...
  convert_drawflow_to_block(name, blockGraph) {
    ...
    for (const block in graph_with_connections) {
      ...
      graph_with_connections[block].views.node.pos_x =
        graph_with_connections[block].views.node.pos_x.toString();
      graph_with_connections[block].views.node.pos_y =
        graph_with_connections[block].views.node.pos_y.toString();
