...
async function generateDistinctId() {

	try {
	
		const [macAddress, key] = await getMAC();
		...
		const result = [macAddress, key, macAsBigInt.toString(), distinctId]
...
// 	const [macAddress, key] = Promise.resolve(getMAC(iface));
// 	let macAsBigInt = BigInt(`0x${macAddress.split(":").join("")}`);
