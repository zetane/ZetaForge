export default class Drawflow {
  ...
  load_block(pipeline) {
    ...
    for (const key in nodes) {
      const id = nodes[key].information.id
      const coreId = id.substring(0, id.lastIndexOf("-"))
      nodes[key].information.id = coreId
      this.addNode_from_JSON(nodes[key], nodes[key].views.node.pos_x, nodes[key].views.node.pos_y);
    }
    for (const key in nodes) {
  ...
  import_block(data, notifi = true) {
    ...
    const pipeline = JSON.parse(JSON.stringify(data));
    this.load_block(pipeline);
    if (notifi) {
